...
- The canonical permission implementation currently lets `MANAGE_WORK` inherit the complete review/sign-off capability bundle, while the approved permissions matrix describes independent approval and propose-only distinctions for several roles. The UI follows the server's effective permissions and does not create an authorisation bypass, but production activation requires an approved server-role reconciliation and segregation-of-duties regression suite.
