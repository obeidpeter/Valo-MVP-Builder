...
7. **Database proof:** both checked-in Replit production run paths now converge on the same-process startup wrapper, which awaits the pinned and serialized `0003`-`0005` launcher before importing the API. The direct/session-affine owner endpoint, six-entry live journal, least-privilege runtime attestation, idempotency, retention and shared limiter behaviour still require evidence from the promoted production candidate.
