...
export function validateReplitMigrationJournal(rows) {
  ...
  if (![3, 6, 7, EXPECTED_REPLIT_MIGRATIONS.length].includes(rows.length)) {
    fail(
      "production journal is not the approved 0000-0002, 0000-0005, 0000-0006, or 0000-0007 state",
    );
  }
...
export function validateReplitIntakeSchemaState(mode, schemaName) {
  ...
  if (
    ["upgrade_pending", "storage_upgrade_pending", "already_applied"].includes(
      mode,
    ) &&
    !present
  ) {
...
export function shouldApplyReplitMigrations(mode) {
  return ["pending", "upgrade_pending", "storage_upgrade_pending"].includes(
    mode,
  );
}
...
export async function validateLocalReplitMigrationManifest() {
  ...
  if (
    ...
    fail("source migration journal is not the frozen eight-entry manifest");
  }
