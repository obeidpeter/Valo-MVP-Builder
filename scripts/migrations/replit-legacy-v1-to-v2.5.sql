...
BEGIN
  ...
  JOIN pg_class AS source_relation ON source_relation.oid = constraint.conrelid
...
BEGIN
  ...
  IF EXISTS (
    ...
    JOIN pg_namespace AS namespace ON namespace.oid=relation.relnamespace
    WHERE namespace.nspname='public' AND relation.relowner=role_oid
  ) THEN
    RAISE EXCEPTION 'runtime login must own zero public relations';
  END IF;
  ...
  IF has_schema_privilege(role_name, 'public', 'CREATE')
     ...
     OR has_sequence_privilege(role_name, 'public.audit_events_row_no_seq', 'UPDATE') THEN
