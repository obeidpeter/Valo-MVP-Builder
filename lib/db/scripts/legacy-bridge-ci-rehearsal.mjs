...
async function assertTenantGuardCatalogInSnapshot(client) {
  ...
  const functionProofs = await client.query(
    `SELECT routine.proname::text AS function_name,
       ...
       pg_catalog.pg_get_userbyid(routine.proowner)='valo_app_runtime'
         AS owner_is_runtime,
       has_function_privilege('valo_app_runtime',routine.oid,'EXECUTE')
...
async function assertTenantGuardCatalog(client) {
  await client.query(
