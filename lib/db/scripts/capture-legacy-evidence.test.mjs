...
test("legacy columns classify only the two pinned full fingerprints", async () => {
  ...
  const replit = legacyColumnsForLineage(
    artifact.legacyColumns,
    LEGACY_LINEAGE_DOCUMENTS_PRE_EXTRACTION,
  );
  ...
  assert.deepEqual(classifyLegacyColumnMap(replit, artifact.legacyColumns), {
    id: LEGACY_LINEAGE_DOCUMENTS_PRE_EXTRACTION,
    columnFingerprintAlgorithm: LEGACY_COLUMN_FINGERPRINT_ALGORITHM,
    columnFingerprintSha256: legacyColumnFingerprint(replit),
  });

  const arbitraryIntersection = new Map(
    [...replit].map(([table, columns]) => [table, [...columns]]),
  );
