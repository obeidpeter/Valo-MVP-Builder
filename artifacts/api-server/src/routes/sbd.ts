...
import {
  serializeSbdTemplate,
  serializeSbdAnnotation,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    const rows = await db
      ...
      .where(
        getOrganisationId(req)
          ? eq(sbdTemplates.organisationId, getOrganisationId(req)!)
          : undefined,
      )
    ...
    res.json(
      rows.map((r) =>
        serializeSbdTemplate(r.template, {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const [created] = await db
      .insert(sbdTemplates)
      .values({ ...parsed.data, organisationId: getOrganisationId(req) })
      .returning();
    await writeAudit({
    ...
    const updated = await db.transaction(async (tx) => {
      if (activating) {
        await tx
          ...
          .where(
            and(
              eq(sbdTemplates.code, existing.code),
              eq(sbdTemplates.status, "active"),
              getOrganisationId(req)
                ? eq(sbdTemplates.organisationId, getOrganisationId(req)!)
                : undefined,
            ),
      ...
      const [row] = await tx
        ...
        .set({
          ...parsed.data,
          optimisticLockVersion: sql`${sbdTemplates.optimisticLockVersion} + 1`,
          updatedAt: new Date(),
        })
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const [{ maxVersion }] = await db
      .select({
        maxVersion: sql<number>`coalesce(max(${sbdTemplates.version}), 0)::int`,
      })
      .from(sbdTemplates)
      .where(
        and(
          eq(sbdTemplates.code, source.code),
          getOrganisationId(req)
            ? eq(sbdTemplates.organisationId, getOrganisationId(req)!)
            : undefined,
        ),
    ...
    const [created] = await db
      .insert(sbdTemplates)
      .values({
        organisationId: source.organisationId ?? getOrganisationId(req),
        code: source.code,
        title: source.title,
