...
test("accepts only the exact approved baseline, upgrade, and complete journal states", () => {
  ...
  for (const rejectedRows of [
    ...
    journalRows(4),
    journalRows(5),
    [...journalRows(7), { id: 8, hash: "0".repeat(64), createdAt: "0" }],
  ]) {
    assert.throws(
      () => validateReplitMigrationJournal(rejectedRows),
      /not the approved 0000-0002, 0000-0005, or 0000-0006 state/,
    );
  }
  assert.throws(
    () =>
...
test("applies migrations for the baseline and six-row upgrade states", () => {
