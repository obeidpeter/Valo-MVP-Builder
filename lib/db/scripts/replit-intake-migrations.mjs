...
export function validateReplitMigrationJournal(rows) {
  ...
  if (![3, EXPECTED_REPLIT_MIGRATIONS.length].includes(rows.length)) {
    fail("production journal is not the approved 0000-0002 or 0000-0005 state");
  }
  ...
  return rows.length === EXPECTED_REPLIT_MIGRATIONS.length
    ? "already_applied"
    : "pending";
}
...
export function validateReplitIntakeSchemaState(mode, schemaName) {
  ...
  if (mode === "already_applied" && !present) {
...
function canonicalIntakeSecurity(catalog) {
  if (
    ...
    catalog.functions.length !== 4 ||
    !Array.isArray(catalog.functionGrants) ||
    catalog.functionGrants.length !== 6
  ) {
    return null;
  }
  return {
...
export async function validateLocalReplitMigrationManifest() {
  ...
  if (
    ...
    fail("source migration journal is not the frozen six-entry manifest");
  }
  ...
  const columns = await client.query(
    `SELECT relation.relname, attribute.attnum, attribute.attname,
       ...
       CASE WHEN attribute.attcollation=0 THEN ''
         ELSE collation_namespace.nspname || '.' || collation.collname END
         AS collation,
  ...
  const indexes = await client.query(
    `SELECT relation.relname, index_relation.relname AS index_name,
       ...
       ARRAY(
         ...
         LEFT JOIN pg_catalog.pg_collation AS collation
           ON collation.oid=selected_collation.collation_oid
         LEFT JOIN pg_catalog.pg_namespace AS collation_namespace
           ON collation_namespace.oid=collation.collnamespace
...
export async function runReplitIntakeMigrations(environment = process.env) {
  ...
  try {
    ...
    process.stdout.write(
      `Replit intake migration gate passed: ${mode === "pending" ? "0003-0005 applied" : "already current"}\n`,
    );
  } finally {
