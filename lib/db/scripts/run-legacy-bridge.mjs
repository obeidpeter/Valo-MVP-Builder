...
const retentionCompletionMigrationPath = resolve(
  repositoryRoot,
  "lib/db/migrations/0011_retention_completion.sql",
);
const deliverySourceReleaseBoundaryMigrationPath = resolve(
  repositoryRoot,
  "lib/db/migrations/0012_delivery_source_release_boundary.sql",
);
const DELIVERY_SOURCE_RELEASE_BOUNDARY_FUNCTIONS = Object.freeze([
...
const RETENTION_COMPLETION_TABLES = Object.freeze([
...
const TENANT_PARENT_EDGE_FUNCTIONS = Object.freeze({
  base: Object.freeze({
...
const RETENTION_COMPLETION_SECURITY_FUNCTIONS = Object.freeze([
  Object.freeze({
...
IN ACCESS EXCLUSIVE MODE NOWAIT`;
const LOCK_RETENTION_COMPLETION_EVIDENCE_TABLES = `LOCK TABLE
...
async function assertPrecommitSecurityCatalog(client) {
  ...
  const bridgeState = await client.query(`
    SELECT production_assurance_expected, retrieval_registry_expected,
      tender_context_expected, retention_completion_expected
  ...
  const retentionCompletionExpected =
    bridgeState.rows[0]?.retention_completion_expected;
  assert.equal(
    typeof retentionCompletionExpected,
    "boolean",
    "bridge retention-completion variant state must be boolean",
  );
  assert(
  ...
  const tenantParentEdges = await client.query(`
  ...
  `);
  assert.equal(
    tenantParentEdges.rowCount,
    retentionCompletionExpected ? 118 : tenderContextExpected ? 116 : 98,
    "pre-commit tenant-parent edge count drift",
  );
  assert.equal(
    sha256(
      tenantParentEdges.rows
        .map(
          (edge) =>
    ...
    "pre-commit tenant-parent edge manifest drift",
  );
  ...
  const expectedRlsCount =
    (productionAssuranceExpected ? 86 : 85) +
    (tenderContextExpected ? 7 : 0) +
    (retentionCompletionExpected ? 1 : 0);
  assert.deepEqual(
    {
  ...
  assert.deepEqual(
    ...
    retentionCompletionExpected
      ...
      : [],
    "pre-commit retention-completion FORCE RLS drift",
  );
  ...
  assert.deepEqual(
    ...
    retentionCompletionExpected
      ...
      : [],
    "pre-commit retention-completion RLS policy drift",
  );
  ...
  assert.deepEqual(securityInventory.rows[0], {
    ...
    security_functions:
      (productionAssuranceExpected ? 11 : 9) +
      (tenderContextExpected ? 3 : 0) +
      (retentionCompletionExpected ? 3 : 0),
  });
  const retentionCompletionTriggers = await client.query(`
    SELECT relation.relname::text AS table_name,
    ...
    WHERE relation_namespace.nspname='public'
      ...
      AND routine.proname='enforce_retention_completion_transition'
      AND NOT guard.tgisinternal
    ORDER BY table_name,trigger_name
  `);
  assert.deepEqual(
    retentionCompletionTriggers.rows,
    retentionCompletionExpected
      ? [
          ["deletion_certificates", "deletion_certificate_completion_guard"],
          ["retention_actions", "retention_action_completion_guard"],
          [
            "retention_action_storage_events",
            "retention_action_storage_event_completion_guard",
          ],
          ["retention_requests", "retention_request_completion_guard"],
        ]
      ...
      : [],
    "pre-commit retention-completion trigger catalog drift",
  );
  ...
  const expectedEdgeFunction = retentionCompletionExpected
    ? TENANT_PARENT_EDGE_FUNCTIONS.retention
    : tenderContextExpected
      ? TENANT_PARENT_EDGE_FUNCTIONS.tender
      : TENANT_PARENT_EDGE_FUNCTIONS.base;
  const edgeFunctions = [
    expectedEdgeFunction,
    ...(retentionCompletionExpected
      ? [TENANT_PARENT_EDGE_FUNCTIONS.retentionBase]
      : []),
  ];
  for (const expected of edgeFunctions) {
    const actual = securityFunctions.rows.find(
      (routine) => routine.function_name === expected.function_name,
    );
    assert(actual, `missing pre-commit ${expected.function_name}`);
  ...
  if (retentionCompletionExpected) {
    for (const expected of RETENTION_COMPLETION_SECURITY_FUNCTIONS) {
      const actual = securityFunctions.rows.find(
        (routine) => routine.function_name === expected.function_name,
      );
      assert(actual, `missing pre-commit ${expected.function_name}`);
...
function withoutBridgeOnlyExtension(
  value,
  startMarker,
  endMarker,
  normalizeBase = true,
  ...
  const base = `${value.slice(0, startLine)}${value.slice(afterEndLine)}`;
  return {
    base: normalizeBase ? normalizeSql(base) : base,
...
async function checkArtifact() {
  const [
    ...
    [
      ...
      retentionCompletionMigrationPath,
      deliverySourceReleaseBoundaryMigrationPath,
  ...
  for (const expected of RETENTION_COMPLETION_SECURITY_FUNCTIONS) {
    const functionStart = retentionCompletionMigration.indexOf(
      `CREATE FUNCTION valo_security.${expected.function_name}(`,
    );
    const sourceStart = retentionCompletionMigration.indexOf(
      "AS $function$",
      functionStart,
    );
    const sourceEnd = retentionCompletionMigration.indexOf(
      "$function$;",
      sourceStart,
    );
    assert(
      functionStart >= 0 &&
        sourceStart > functionStart &&
        sourceEnd > sourceStart,
      `0011 is missing ${expected.function_name}`,
    );
    assert.equal(
      sha256(
        normalizeSql(
          retentionCompletionMigration.slice(
            sourceStart + "AS $function$".length,
            sourceEnd,
          ),
        ),
      ...
      expected.source_sha256,
      `0011 ${expected.function_name} source hash drift`,
    );
  }
  assert.deepEqual(
    [
      ...deliverySourceReleaseBoundaryMigration.matchAll(
        /CREATE OR REPLACE FUNCTION public\.([a-z0-9_]+)\s*\(/gu,
      ),
    ]
      .map((match) => match[1])
      .sort(),
    DELIVERY_SOURCE_RELEASE_BOUNDARY_FUNCTIONS,
    "0012 public-function inventory drift",
  );
  assert.deepEqual(
    [
      ...deliverySourceReleaseBoundaryMigration.matchAll(
        /CREATE TRIGGER delivery_source_project_guard\s+BEFORE INSERT OR UPDATE OR DELETE ON public\.([a-z0-9_]+)\s+FOR EACH ROW EXECUTE FUNCTION public\.valo_guard_delivery_source_mutation\(\);/gu,
      ),
    ]
      .map((match) => match[1])
      .sort(),
    DELIVERY_SOURCE_RELEASE_BOUNDARY_TABLES,
    "0012 guarded-table inventory drift",
  );
  assert.equal(
    [...deliverySourceReleaseBoundaryMigration.matchAll(/CREATE TRIGGER /gu)]
      .length,
    DELIVERY_SOURCE_RELEASE_BOUNDARY_TABLES.length + 1,
    "0012 contains an unpinned trigger",
  );
  assert.match(
    deliverySourceReleaseBoundaryMigration,
    ...
    "0012 project-delete cascade guard drift",
  );
  ...
  assert.match(
    precommitSecurityCatalog,
    /deliverySourceReleaseBoundaryExpected/u,
  );
  assert.match(precommitSecurityCatalog, /trigger_count:\s*14/u);
  assert.match(precommitSecurityCatalog, /project_delete_trigger_count:\s*1/u);
  assert.match(
    precommitSecurityCatalog,
    /retentionCompletionExpected \? 3 : 0/u,
  );
  assert.match(
    bridge,
    /IF NOT \(SELECT retention_completion_expected FROM _valo_bridge_state\)[\s\S]*AND EXISTS \([\s\S]*created_at=1787472000000[\s\S]*0011 retention-completion journal evidence exists without its catalog/u,
    "a pre-0011 catalog must reject a stray 0011 journal row",
  );
  assert.match(
    bridge,
    /delivery_source_release_boundary_expected[\s\S]*created_at=1787558400000[\s\S]*hash=inputs\.migration_0012_hash[\s\S]*0012 delivery-source release-boundary journal evidence is absent or drifted/u,
    "the 0012 catalog must require the exact checked-in migration row",
  );
  assert.match(
    bridge,
    /NOT \(SELECT delivery_source_release_boundary_expected[\s\S]*created_at=1787558400000[\s\S]*0012 delivery-source release-boundary journal evidence exists without its catalog/u,
    "a pre-0012 catalog must reject a stray 0012 journal row",
  );
  assert.equal(
    bridge.split("__VALO_BRIDGE_MIGRATION_0012_HASH__").length - 1,
    1,
    "bridge must contain one 0012 migration-hash input token",
  );
  assert.match(
    bridge,
    /delivery_source_release_boundary_expected[\s\S]*REVOKE DELETE ON TABLE public\.clients FROM %I/u,
    "the bridge must preserve the 0012 client-cascade privilege boundary",
  );
  ...
  const bridge0011EdgeExtensionStart =
    "-- BEGIN BRIDGE-ONLY 0011 TENANT-PARENT PREFLIGHT EXTENSION.";
  const bridge0011EdgeExtensionEnd =
    "-- END BRIDGE-ONLY 0011 TENANT-PARENT PREFLIGHT EXTENSION.";
  const {
    base: embedded0002SecurityWithout0011,
    extension: bridge0011EdgeExtension,
  } = withoutBridgeOnlyExtension(
    embedded0002Security,
    bridge0011EdgeExtensionStart,
    bridge0011EdgeExtensionEnd,
    false,
  );
  const { base: embedded0002SecurityBase, extension: bridge0010EdgeExtension } =
    withoutBridgeOnlyExtension(
      embedded0002SecurityWithout0011,
      bridge0010EdgeExtensionStart,
      bridge0010EdgeExtensionEnd,
    );
  assert.equal(
    sha256(canonicalOperationalSql(embedded0002Security)),
    "60e3de9b40df8f5706e91e0167bbad09b13739a795445ffc6036d25a4e04fcaa",
    "embedded 0002 security plus its 0010/0011 preflight extensions drifted",
  );
  ...
  const migration0011EdgeFunctionStart = retentionCompletionMigration.indexOf(
    "CREATE FUNCTION valo_security.expected_tenant_parent_edges()",
  );
  assert(
    migration0011EdgeFunctionStart >= 0,
    "0011 tenant-parent extension function is absent",
  );
  const migration0011EdgeFunction = retentionCompletionMigration.slice(
    migration0011EdgeFunctionStart,
    retentionCompletionMigration.indexOf(
      "$function$;",
      migration0011EdgeFunctionStart,
    ),
  );
  ...
  const bridge0011Edges = extractEdgeTuples(bridge0011EdgeExtension);
  const migration0011Edges = extractEdgeTuples(migration0011EdgeFunction);
  assert.equal(
    bridge0011Edges.length,
    2,
    "bridge-only 0011 precommit extension must contain exactly two edges",
  );
  assert.deepEqual(
    bridge0011Edges,
    migration0011Edges,
    "bridge-only 0011 precommit extension differs from migration 0011",
  );
  assert.match(
    bridge0011EdgeExtension,
    /WHERE \(SELECT retention_completion_expected FROM _valo_bridge_state\)\s+UNION ALL$/u,
    "bridge-only 0011 edges must be conditional and preserve the archive union",
  );
  assert.equal(
    sha256(canonicalOperationalSql(bridge0011EdgeExtension)),
    "adf4943b07ad5447b2269662b00dceac5c233803e6895d8526c77a49117a0184",
    "bridge-only 0011 precommit operational SQL drifted",
  );
  const bridge0011EdgeManifest = bridge0011Edges
    .map(
      ([childTable, childColumn, parentTable, parentColumn, allowGlobal]) =>
        `${childTable}.${childColumn}->${parentTable}.${parentColumn}|${allowGlobal === "true" ? "global" : "strict"}`,
    )
    ...
    .join("\n");
  assert.equal(
    sha256(bridge0011EdgeManifest),
    "505abea4552f8551a3942a3b953f8a763e49094fb3ce6394666a69ae0612e4c4",
    "bridge-only 0011 edge manifest drifted",
  );
  const restoreStart = bridge.indexOf(
    "DO $restore_additive_tenant_parent_edges$",
  );
  const restoreEnd = bridge.indexOf(
    "$restore_additive_tenant_parent_edges$;",
    restoreStart,
  );
  assert(
    restoreStart >= 0 && restoreEnd > restoreStart,
    "additive tenant-parent restoration block is absent",
  );
  const restoreBlock = bridge.slice(restoreStart, restoreEnd);
  const extractFunctionSource = (source, searchStart = 0) => {
    const sourceStart = source.indexOf("AS $function$", searchStart);
    const sourceEnd = source.indexOf("$function$", sourceStart + 13);
    assert(sourceStart >= 0 && sourceEnd > sourceStart);
    return normalizeSql(source.slice(sourceStart + 13, sourceEnd));
  };
  const restoredRetentionSource = extractFunctionSource(restoreBlock);
  const restoredTenderSource = extractFunctionSource(
    restoreBlock,
    restoreBlock.indexOf(
      "$function$",
      restoreBlock.indexOf("AS $function$") + 13,
    ) + 10,
  );
  assert.equal(
    sha256(restoredRetentionSource),
    TENANT_PARENT_EDGE_FUNCTIONS.retention.source_sha256,
    "completed 0011 target must restore the exact current edge wrapper",
  );
  assert.equal(
    sha256(restoredTenderSource),
    TENANT_PARENT_EDGE_FUNCTIONS.tender.source_sha256,
    "completed 0010 target must restore the exact tender edge wrapper",
  );
  assert.match(
    restoredRetentionSource,
    /FROM valo_security\.expected_tenant_parent_edges_v10\(\)/u,
    "0011 restoration must preserve the v25 -> v10 -> current hierarchy",
  );
  assert.equal(
  ...
  return {
    ...
    retentionCompletionMigrationHash: sha256(retentionCompletionMigration),
    deliverySourceReleaseBoundaryMigrationHash: sha256(
      deliverySourceReleaseBoundaryMigration,
    ),
  };
}
...
async function main() {
  ...
  try {
    ...
    if (targetState === "complete") {
      ...
      const retentionCompletionTable = await owner.query(
        "SELECT to_regclass('public.retention_action_storage_events') IS NOT NULL AS present",
      );
      if (retentionCompletionTable.rows[0]?.present) {
        await owner.query(LOCK_RETENTION_COMPLETION_EVIDENCE_TABLES);
      }
      await owner.query(
    ...
    const replacements = new Map([
      ...
      [
        ...
        artifact.retentionCompletionMigrationHash,
      ],
      [
        "__VALO_BRIDGE_MIGRATION_0012_HASH__",
        artifact.deliverySourceReleaseBoundaryMigrationHash,
      ],
    ]);
  ...
  try {
    const deliverySourceReleaseBoundaryExpected = (
      await runtimeClient.query(`SELECT pg_catalog.to_regprocedure(
        'public.valo_guard_delivery_project_delete()'
      ) IS NOT NULL AS expected`)
    ).rows[0]?.expected;
    const assertInvisibleClientDelete = async (clientId, message) => {
      if (!deliverySourceReleaseBoundaryExpected) {
        assert.equal(
          (
      ...
      await runtimeClient.query("SAVEPOINT client_delete_privilege_check");
      let denied = false;
      try {
        await runtimeClient.query(
          "DELETE FROM public.clients WHERE id=$1::uuid",
      ...
      await runtimeClient.query(
        "ROLLBACK TO SAVEPOINT client_delete_privilege_check",
      );
      await runtimeClient.query(
        "RELEASE SAVEPOINT client_delete_privilege_check",
      );
    ...
    await assertInvisibleClientDelete(
      tenantClient.rows[0].id,
