...
async function seedLegacyFixture(client, legacyShape) {
  ...
  for (const column of Object.keys(document)) {
    if (!legacyShape.get("documents").includes(column)) delete document[column];
  }
  await insertRow(client, "documents", document);
...
async function main() {
  ...
  try {
    ...
    const manifest = {
      ...
      componentManifestSha256: sha256(
        JSON.stringify({
          ...
          legacyLineage: requestedLineage,
    ...
    try {
      ...
      if (requestedLineage === LEGACY_LINEAGE_DOCUMENTS_PRE_EXTRACTION) {
        const upgradedDocument = await bridged.query(
          `SELECT sha256, extraction_method, extraction_confidence,
             extraction_notes
           FROM public.documents WHERE id=$1::uuid`,
          [IDS.document],
        );
        assert.deepEqual(upgradedDocument.rows, [
          {
            sha256: sha256("synthetic tender fixture"),
