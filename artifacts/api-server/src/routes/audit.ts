...
router.get("/audit/access-review", requireMember, async (req: Request, res: Response) => {
  const window = monthWindow(req.query.month as string | undefined);
  if (!window) {
    res.status(400).json({ error: "month query parameter is required in YYYY-MM format" });
    return;
  }
  ...
  const rows = await db
    ...
    .where(and(gte(auditEvents.createdAt, window.start), lt(auditEvents.createdAt, window.end)))
    .orderBy(desc(auditEvents.createdAt));
  ...
  const accessRows = rows
    ...
    .map((r) => ({
      at: r.audit.createdAt instanceof Date ? r.audit.createdAt.toISOString() : String(r.audit.createdAt),
  ...
  if (req.query.format === "csv") {
    const headers = ["at", "actor", "client", "project", "action", "objectType", "objectId", "details"];
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="access-review-${req.query.month}.csv"`);
    res.send([headers.join(","), ...accessRows.map((row) => headers.map((h) => csvEscape(row[h as keyof typeof row])).join(","))].join("\n"));
    return;
  }
