...
export const EXPECTED_REPLIT_MIGRATIONS = Object.freeze([
  ...
  Object.freeze({
    id: 10,
    tag: "0009_ai_retrieval_registry",
    createdAt: "1786752000000",
    hash: "8bf822170bd0adcb91a5020153100de35b463c179fa713781bcecadd39b61767",
  }),
  Object.freeze({
    id: 11,
    tag: "0010_tender_context_and_addendum",
    createdAt: "1787385600000",
    hash: "8cbfb2fb67bc4bacca76073f9b1f0a72511ba95fd62016f53c237f0ee193bcf5",
  }),
  Object.freeze({
    id: 12,
    tag: "0011_retention_completion",
    createdAt: "1787472000000",
    hash: "02aa2719244fd9cf2cdd9eed8a96e77af4332e6ce333bd9296d6290ea3b6b239",
  }),
  Object.freeze({
    id: 13,
    tag: "0012_delivery_source_release_boundary",
    createdAt: "1787558400000",
    hash: "29549e6032518ba17d87e9b2273f3cf122221e9a30031d03c13ba52382308574",
  }),
  Object.freeze({
    id: 14,
    tag: "0013_delivery_guard_least_privilege",
    createdAt: "1787644800000",
    hash: "afc65da833edac9f0622e8ba46a0ca80c9dbf5dd1011c1c882efdf4fa86689b7",
  }),
]);
...
export function validateReplitMigrationJournal(rows) {
  ...
  if (
    ![
      ...
      12,
      13,
      EXPECTED_REPLIT_MIGRATIONS.length,
    ].includes(rows.length)
  ) {
    fail("production journal is not an approved prefix through 0013");
  }
...
export function validateReplitMigrationSearchPath(proof) {
  const lookupPath = proof?.lookupPath;
  if (
    proof?.configuredPath !== "public, pg_temp" ||
    proof?.currentSchema !== "public" ||
    !Array.isArray(lookupPath) ||
    lookupPath[0] !== "pg_catalog" ||
    lookupPath[1] !== "public" ||
    lookupPath.slice(2).some((schema) => !/^pg_temp_[0-9]+$/u.test(schema))
  ) {
    fail("migration search path is not the approved catalog/public boundary");
  }
}

export async function withReplitMigrationSearchPath(client, migratePending) {
  await client.query("SET search_path = public, pg_temp");
  try {
    const proof = await client.query(`
      SELECT pg_catalog.current_setting('search_path')::pg_catalog.text AS "configuredPath",
        pg_catalog.current_schema()::pg_catalog.text AS "currentSchema",
        pg_catalog.current_schemas(true)::pg_catalog.text[] AS "lookupPath"
    `);
    validateReplitMigrationSearchPath(proof.rows[0]);
    return await migratePending();
  } finally {
...
export async function validateLocalReplitMigrationManifest() {
  ...
  if (
    ...
    fail("source migration journal is not the frozen fourteen-entry manifest");
  }
...
export async function runReplitIntakeMigrations(environment = process.env) {
  ...
  try {
    ...
    if (shouldApplyReplitMigrations(mode)) {
      await withReplitMigrationSearchPath(lockClient, () =>
        ...
        : mode === "upgrade_pending"
          ...
          : mode === "storage_upgrade_pending"
            ...
            : mode === "assurance_upgrade_pending"
              ? "0008-0013 applied"
              : mode === "registry_upgrade_pending"
                ? "0009-0013 applied"
                : mode === "tender_context_upgrade_pending"
                  ? "0010-0013 applied"
                  : mode === "retention_completion_upgrade_pending"
                    ? "0011-0013 applied"
                    : mode ===
                        "delivery_source_release_boundary_upgrade_pending"
                      ? "0012-0013 applied"
