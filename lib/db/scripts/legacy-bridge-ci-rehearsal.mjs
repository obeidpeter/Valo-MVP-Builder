...
async function assertRuntimeContract(client, { productionAssuranceExpected }) {
  assert.equal(
...
async function assertTenantGuardCatalogInSnapshot(
  client,
  { runtimeRoleExpected, productionAssuranceExpected },
) {
  ...
  assert.equal(
    sha256(
      rlsCatalog.rows
        .filter((row) => row.table_name !== "authenticated_rate_limit_buckets")
...
async function assertTenantGuardCatalog(
  client,
  { runtimeRoleExpected = true, productionAssuranceExpected = true } = {},
) {
  await client.query(
...
async function main() {
  ...
  const expectedJournal = [
    ...
    1786339224638, 1786425600000, 1786622400000, 1786708800000,
  ].map((createdAt, index) => ({
  ...
  try {
    ...
    try {
      await assertRuntimeContract(fresh, {
        productionAssuranceExpected: true,
      });
