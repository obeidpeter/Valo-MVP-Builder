...
const LEGACY_LINEAGE_DOCUMENTS_PRE_EXTRACTION =
  "replit-legacy-v1-documents-pre-extraction";
const LEGACY_LINEAGE_IDS = [
  LEGACY_LINEAGE_CANONICAL,
  LEGACY_LINEAGE_DOCUMENTS_PRE_EXTRACTION,
];
const REPLIT_DOCUMENTS_PRE_EXTRACTION_COLUMNS = [
  "id",
  "project_id",
  "type",
  "filename",
...
function legacyColumnsForLineage(canonicalColumns, lineageId) {
  ...
  const columns = new Map(
    LEGACY_TABLES.map((table) => {
      const canonical = canonicalColumns.get(table);
      assert(
        Array.isArray(canonical),
        `canonical columns are absent for ${table}`,
      );
      return [table, [...canonical]];
    }),
  );
  if (lineageId === LEGACY_LINEAGE_DOCUMENTS_PRE_EXTRACTION) {
    columns.set("documents", [...REPLIT_DOCUMENTS_PRE_EXTRACTION_COLUMNS]);
  }
  return columns;
}
...
async function checkArtifact() {
  ...
  const variantColumnsMatch = effectiveColumnSection.match(
    /THEN ARRAY\[([^\]]+)\]/,
  );
  assert(variantColumnsMatch, "pinned Replit documents lineage is absent");
  const variantColumns = [...variantColumnsMatch[1].matchAll(/'([^']+)'/g)].map(
    (entry) => entry[1],
  );
  assert.deepEqual(
    variantColumns,
    REPLIT_DOCUMENTS_PRE_EXTRACTION_COLUMNS,
    "SQL and runner Replit documents lineages differ",
  );
  assert.equal(
