...
import { Loader2, Calculator, Plus, Upload, FileSpreadsheet, X } from "lucide-react";
...
function detectDelimiter(text: string): string {
  ...
  const counts: [string, number][] = ["\t", ",", ";"].map((d) => [
    d,
    (firstLines.match(new RegExp(d === "\t" ? "\t" : `\\${d}`, "g")) ?? []).length,
  ]);
...
const FIELDS = [
  { key: "lineRef", label: "Item Ref", pattern: /^(item\s*(no|ref)?|ref|s\/?n|no\.?|line)/i },
  { key: "description", label: "Description", pattern: /desc|particular|work item|activity/i },
  { key: "quantity", label: "Quantity", pattern: /^qty|quant/i },
  { key: "unitRate", label: "Unit Rate", pattern: /rate|unit\s*(price|cost)/i },
  { key: "extension", label: "Amount / Extension", pattern: /amount|extension|^ext|total(?!s)/i },
...
function autoMap(header: string[]): Record<FieldKey, string> {
  const mapping = Object.fromEntries(FIELDS.map((f) => [f.key, UNMAPPED])) as Record<FieldKey, string>;
  const taken = new Set<number>();
  for (const field of FIELDS) {
    const idx = header.findIndex((h, i) => !taken.has(i) && field.pattern.test(h.trim()));
    if (idx >= 0) {
...
export function BoqTab({ projectId }: { projectId: string }) {
  ...
  const [mapping, setMapping] = useState<Record<FieldKey, string>>(
    Object.fromEntries(FIELDS.map((f) => [f.key, UNMAPPED])) as Record<FieldKey, string>,
  );
  ...
  const loadGrid = (g: Grid, name: string) => {
    if (g.length === 0) {
      toast({ variant: "destructive", title: "No rows found in the spreadsheet" });
      return;
    }
    setGrid(g);
    setSourceName(name);
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (/\.(xlsx|xls)$/i.test(file.name)) {
        ...
        const rows = xlsxUtils.sheet_to_json<unknown[]>(ws, { header: 1, raw: false, defval: "" });
        loadGrid(
          rows.map((r) => r.map((c) => String(c ?? ""))).filter((r) => r.some((c) => c.trim() !== "")),
          file.name,
        );
      } else {
      ...
      toast({ variant: "destructive", title: "Could not parse the spreadsheet file" });
    } finally {
  ...
  const columnLabel = (i: number) =>
    grid && hasHeader && grid[0][i]?.trim() ? grid[0][i].trim() : `Column ${i + 1}`;
  ...
  const handleRunChecks = () => {
    const rows = dataRows
      ...
      .filter(
        (r) =>
          r.lineRef || r.description || r.quantity != null || r.unitRate != null || r.extension != null,
      );
    if (rows.length === 0) {
    ...
    runChecks.mutate(
      ...
      },
      {
        ...
        onError: () => toast({ variant: "destructive", title: "BOQ check run failed" }),
      },
    );
  };
    ...
    <div className="space-y-6">
      ...
      {!grid ? (
        <div className="bg-card border border-border p-4 rounded-lg shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Upload a CSV or Excel file, or paste rows straight from a spreadsheet. You'll map the columns before checks run.
              </p>
            </div>
            <input
              type="file"
          ...
          />
          <Button onClick={handlePasteLoad} disabled={!pasteData.trim()} variant="outline">
        ...
        <div className="bg-card border border-border p-4 rounded-lg shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-medium">Map Columns — {sourceName}</h3>
              <p className="text-xs text-muted-foreground mt-0.5">
                {dataRows.length} data row(s). Confirm which column feeds each check field; unmapped fields are skipped.
              </p>
            </div>
            <Button variant="ghost" size="icon" title="Discard loaded data" onClick={() => setGrid(null)}>
              <X className="w-4 h-4" />
            </Button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {FIELDS.map((f) => (
              <div key={f.key} className="space-y-1">
                ...
                <Select
                  value={mapping[f.key]}
                  onValueChange={(val) => setMapping({ ...mapping, [f.key]: val })}
                >
          ...
          <div className="border border-border rounded-md overflow-x-auto">
              ...
              <tbody>
                {dataRows.slice(0, 6).map((r, ri) => (
                  <tr key={ri} className="border-b border-border last:border-0">
                    {Array.from({ length: Math.min(columnCount, 10) }, (_, ci) => (
                      <td key={ci} className="px-2 py-1.5 whitespace-nowrap max-w-[180px] overflow-hidden text-ellipsis">
                        {r[ci] ?? ""}
                      </td>
                    ))}
                  </tr>
            ...
            {dataRows.length > 6 && (
              <div className="px-2 py-1 text-[10px] text-muted-foreground">+{dataRows.length - 6} more row(s)</div>
            )}
          </div>

          <div className="flex flex-wrap items-end gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-muted-foreground uppercase">Declared Grand Total (optional)</label>
              <Input
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-muted-foreground uppercase" title="Zero by default: one kobo of drift is a finding.">
                Tolerance (optional, ₦)
              </label>
              <Input
