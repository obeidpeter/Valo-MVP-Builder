...
async function assertPrecommitSecurityCatalog(client) {
  ...
  const expectedRlsCount = productionAssuranceExpected ? 86 : 85;
  assert.deepEqual(
    {
  ...
  assert.deepEqual(securityInventory.rows[0], {
    public_triggers: 116,
    security_functions: productionAssuranceExpected ? 11 : 9,
  });
...
async function checkArtifact() {
  const [
    ...
    [bridgePath, ...migrationPaths, productionAssuranceMigrationPath].map(
