...
import { getActiveConfigRow, toActiveConfig, APP_CONFIG_ID } from "../lib/appConfig";
...
router.get("/config", requireRoles("admin"), async (_req: Request, res: Response) => {
  const row = await getActiveConfigRow();
  res.json(serializeConfig(row));
});

router.patch("/config", requireRoles("admin"), async (req: Request, res: Response) => {
  ...
  if (numerics.some((n) => n !== undefined && !Number.isInteger(n))) {
    res.status(400).json({ error: "Numeric configuration values must be integers." });
    return;
  }
  ...
  const critical = d.bandCutoffs?.critical ?? current.risk.bandCutoffs.critical;
