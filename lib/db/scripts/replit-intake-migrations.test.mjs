...
test(
  "applies 0007-0013 from the exact seven-entry Replit production prefix",
  { timeout: 180_000 },
  async (context) => {
    ...
    try {
      ...
      try {
        ...
        `);
        const ownerOnlyAcl = ["$OWNER>$OWNER:EXECUTE:false"];
        const helperAcl = [
          ...ownerOnlyAcl,
          "$OWNER>$ROLE:valo_app_runtime:EXECUTE:false",
        ];
        assert.deepEqual(
          deliveryGuardFunctions.rows.map((row) => ({
            ...row,
            executeAcl: [...row.executeAcl].sort(),
          })),
          [
            {
