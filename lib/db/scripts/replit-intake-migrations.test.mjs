...
test("accepts only the exact approved baseline, upgrade, and complete journal states", () => {
  ...
  for (const rejectedRows of [
    ...
    journalRows(4),
    journalRows(5),
    [...journalRows(9), { id: 10, hash: "0".repeat(64), createdAt: "0" }],
  ]) {
    assert.throws(
      () => validateReplitMigrationJournal(rejectedRows),
      /not the approved 0000-0002, 0000-0005, 0000-0006, 0000-0007, or 0000-0008 state/,
    );
  }
  assert.throws(
    () =>
  ...
  const client = {
    query: async (statement) => {
      ...
      return statement.includes("current_schema")
        ? {
            rows: [
              { currentSchema: "public", lookupPath: ["pg_catalog", "public"] },
            ],
          }
        : { rows: [] };
    },
...
test("keeps catalog-only postchecks after the scoped migration path", () => {
  assert.match(
    ...
    /export async function withReplitMigrationSearchPath[\s\S]*SET search_path = public, pg_temp[\s\S]*current_schema[\s\S]*current_schemas\(true\)[\s\S]*finally[\s\S]*SET search_path = pg_catalog/u,
  );
  assert.match(
    replitMigrationRunner,
...
test(
  "applies 0007-0008 from the exact seven-entry Replit production prefix",
  { timeout: 180_000 },
  async (context) => {
