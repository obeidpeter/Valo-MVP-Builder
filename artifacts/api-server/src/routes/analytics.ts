...
import { requireMember, requireRoles } from "../middlewares/auth";
 ...
 */
router.get("/projects/:id/cost", requireMember, async (req: Request, res: Response) => {
  ...
  const byTask = new Map<string, { runs: number; promptTokens: number; completionTokens: number }>();
  for (const row of rows) {
    const entry = byTask.get(row.task) ?? { runs: 0, promptTokens: 0, completionTokens: 0 };
    entry.runs += 1;
 ...
 */
router.get("/analytics/cost", requireRoles("admin"), async (req: Request, res: Response) => {
  ...
  const rows = await db
    ...
    .where(and(gte(llmRuns.createdAt, start), lt(llmRuns.createdAt, end)));

  const byProject = new Map<
    string,
    { projectId: string | null; tenderTitle: string | null; runs: number; promptTokens: number; completionTokens: number }
  >();
  for (const row of rows) {
    const key = row.projectId ?? "(no engagement)";
    const entry =
      byProject.get(key) ??
      { projectId: row.projectId, tenderTitle: row.tenderTitle ?? null, runs: 0, promptTokens: 0, completionTokens: 0 };
    entry.runs += 1;
    entry.promptTokens += row.promptTokens ?? 0;
    entry.completionTokens += row.completionTokens ?? 0;
