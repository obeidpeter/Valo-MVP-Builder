...
test("accepts only the exact approved baseline, upgrade, and complete journal states", () => {
  ...
  for (const rejectedRows of [
    ...
    journalRows(4),
    journalRows(5),
    [...journalRows(11), { id: 12, hash: "0".repeat(64), createdAt: "0" }],
  ]) {
    assert.throws(
      () => validateReplitMigrationJournal(rejectedRows),
      /not an approved prefix through 0010/,
    );
  }
  assert.throws(
    () =>
...
test(
  "applies 0007-0010 from the exact seven-entry Replit production prefix",
  { timeout: 180_000 },
  async (context) => {
