...
7. **Database proof:** the source-controlled Replit launcher now pins and serializes the `0003`-`0005` migration suffix before API startup, but the direct/session-affine owner endpoint, six-entry live journal, least-privilege runtime attestation, idempotency, retention and shared limiter behaviour still require evidence from the promoted production candidate.
